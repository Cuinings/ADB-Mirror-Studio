...
> 当前开发版本：`V1.3.2`。个人、组织和企业均可在合法授权的设备上免费使用全部功能。
...
1. 先在“设备”页连接并确认目标设备在线。
2. 打开“镜像与录制”，选择默认质量预设。
3. 如需启动镜像时直接录屏，展开高级选项并指定 `.mkv` 或 `.mp4` 文件。
4. 在在线设备卡片中点击“打开镜像”。
5. 可在“镜像与录制”中查看会话，随时开始/停止录制或停止镜像。
...
1. 打开“文件传输”。
2. 选择目标设备。
3. 在“上传文件”分区选择或拖入本地文件，再推送到设备 Download 目录。
4. 在“从设备下载”分区输入以 `/` 开头的设备绝对路径并选择本地目录。
5. 在“安装 APK”分区选择 APK 并执行覆盖安装。
6. 需要终止上传任务时点击“取消传输”。
...
1. 打开“设备工具”；页面默认沿用“文件传输”中选择的当前设备。
2. 在“常用”分区查看设备详情、保存截图、导出 Logcat 或发送快捷按键。
3. 在“应用管理”分区读取、启动、停止或卸载用户应用。
4. 仅在确认目标设备和命令后进入“高级”分区使用 ADB Shell。
...
- 检查手机端 USB 调试授权。
- 安装设备厂商 USB 驱动。
- 在“设置 → 诊断”检查 ADB 是否正常以及 PATH 中是否存在冲突版本。
...
commercial\artifacts\release\
  AdbMirrorStudio-V1.3.2-win-x64.zip
```
...
commercial\artifacts\installer\
  ADB-Mirror-Studio-Setup-V1.3.2-win-x64.exe
```
